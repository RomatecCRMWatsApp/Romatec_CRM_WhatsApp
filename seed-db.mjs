...
import { contacts, properties, companyConfig } from "./drizzle/schema.js";
import fs from "fs";
...
async function seedDatabase() {
  ...
  try {
    ...
    if (fs.existsSync(contactsPath)) {
      ...
      const contactsToInsert = contactsData.slice(0, 100).map(c => ({
        name: c.name || "Sem nome",
        phone: c.phone || "",
        email: c.email || null,
        status: "active",
      })).filter(c => c.phone); // Filtrar contatos sem telefone
