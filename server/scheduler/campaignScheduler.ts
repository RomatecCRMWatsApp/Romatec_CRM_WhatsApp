...
class CampaignScheduler {
  private state: SchedulerState = {
    isRunning: false, currentCampaignIndex: 0, currentRound: 1,
  ...
  private async assignContactsToCampaign(campaignId: number, count = 2) {
    ...
    await db.update(campaigns).set({ totalContacts: 2 }).where(eq(campaigns.id, campaignId));
  ...
  private generateMessageVariations(prop: any): string[] {
    ...
    const isChacara = name.toLowerCase().includes('chacara') || name.toLowerCase().includes('chacar') || name.toLowerCase().includes('giuliano');
  ...
  private async sendDailyReport() {
    try {
      const report = [`📊 *RELATÓRIO DIÁRIO - ROMATEC*`, `✅ Enviadas: ${this.state.totalSent}`, `❌ Falhas: ${this.state.totalFailed}`, `🚀 Novo ciclo às ${HORA_INICIO}h!`].join('\n');
